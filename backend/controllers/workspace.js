...
const getWorkspaceProjects = async (req, res) => {
  try {
    ...
    const projects = await Project.find({
      ...
      .populate("tasks", "status title dueDate isArchived priority")
      .sort({ createdAt: -1 });
...
const getWorkspaceStats = async (req, res) => {
  try {
    ...
    const totalTaskTesting = projects.reduce((acc, project) => {
      return (
        acc +
        project.tasks.filter((task) => task.status === "Testing").length
      );
    }, 0);
          ...
          if (dayData) {
            switch (task.status) {
              ...
              case "In Progress":
                ...
                break;
              case "Testing":
                dayData.inProgress++; // Count Testing as in progress for trends
                break;
              case "To Do":
                dayData.toDo++;
                break;
    ...
    for (const project of projects) {
      ...
      const projectAge = Math.floor((new Date() - new Date(project.createdAt)) / (1000 * 60 * 60 * 24));

      workspaceProductivityData.push({
        name: project.title.length > 15 ? project.title.substring(0, 15) + "..." : project.title,
        completed: completedTask.length,
        total: projectTask.length,
        efficiency: projectTask.length > 0 ? (completedTask.length / projectTask.length * 100).toFixed(1) : 0,
    ...
    workspaceProductivityData.sort((a, b) => b.efficiency - a.efficiency);
...
const updateWorkspace = async (req, res) => {
  try {
    ...
    await recordActivity(
      ...
      workspaceId,
      {
        description: `Updated ${workspace.name} workspace`,
      }
    );
...
const deleteWorkspace = async (req, res) => {
  try {
    ...
    // Delete all related projects and tasks
